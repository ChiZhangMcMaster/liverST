#' Perform GO enrichment analysis and return grouped circular cnetplot
#'
...
#' @param show_category Number of GO categories to display (recommended 4–6)
#' @param circular Logical; circular layout
#'
#' @return ggplot object
#' @export
plot_go_cnet <- function(
    deg_table,
  ...
  # ---- Remove redundant GO terms (CRITICAL) ----
  ...
  # ---- Compute term similarity for grouping ----
  ego <- enrichplot::pairwise_termsim(ego)

  # ---- Make readable ----
  ego_readable <- clusterProfiler::setReadable(
    ego,
    OrgDb = orgdb,
    keyType = "ENTREZID"
  )

  # ---- Fold change vector (SYMBOL named!) ----
  geneList <- df_sig$log2FoldChange
  names(geneList) <- df_sig$SYMBOL
